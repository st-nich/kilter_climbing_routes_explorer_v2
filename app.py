...
with col1:
    # --- ZOOM CONTROLS ---
    with st.expander("Map Controls (Pan/Zoom)", expanded=False):
        c1, c2, c3, c4, c5 = st.columns(5)
        c1.button("➕", on_click=update_zoom, args=('in',), use_container_width=True)
        c2.button("➖", on_click=update_zoom, args=('out',), use_container_width=True)
        c3.button("⬅️", on_click=update_zoom, args=('left',), use_container_width=True)
        c4.button("➡️", on_click=update_zoom, args=('right',), use_container_width=True)
        c5.button("↺", on_click=update_zoom, args=('reset',), use_container_width=True)
        
        c_up, _, c_down = st.columns([1, 0.1, 1])
        c_up.button("⬆️", on_click=update_zoom, args=('up',), use_container_width=True)
        c_down.button("⬇️", on_click=update_zoom, args=('down',), use_container_width=True)

    # Selection object
    sel = alt.selection_point(name="point_select", on="click", fields=['uuid'], empty=False)

    # Base Chart with manual domain
...
with col2:
    ...
    # --- UPDATED BOARD RENDERER (Sharper + Colored) ---
    def generate_board_svg(uuid, holds_map, layout):
        width, height = 150, 160
        svg = [f'<rect x="0" y="0" width="{width}" height="{height}" fill="#111" rx="4" />']
        
        # Color Map (Kilter Standard)
        # 12=Start(Green), 13=Mid(Cyan), 14=Finish(Magenta), 15=Foot(Orange)
        colors = {
            12: '#00DD00', # Green
            13: '#00FFFF', # Cyan
            14: '#FF00FF', # Magenta
        ...
        # Handle simplified tuple list [(id, role), ...]
        for h_data in route_holds:
            # Check if we have role info
            if isinstance(h_data, (list, tuple)) and len(h_data) == 2:
                h_id, role = h_data
            else:
                h_id, role = h_data, 13 # Default to Cyan
            ...
            if coords:
                ...
                # Glow (Reduced blur for sharpness)
                svg.append(f'<circle cx="{cx}" cy="{cy}" r="4" fill="{c}" opacity="0.5" filter="blur(1px)" />')
                # Core (Sharper, smaller)
                svg.append(f'<circle cx="{cx}" cy="{cy}" r="2" fill="{c}" />')
                # Ring (High contrast)
                svg.append(f'<circle cx="{cx}" cy="{cy}" r="3.5" stroke="{c}" stroke-width="0.8" fill="none" />')
